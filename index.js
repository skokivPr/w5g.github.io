...
function renderDashboard() {
    ...
    const grouped = {};
    workers.forEach(w => {
        const code = (w.shifts[idx] || '').trim().toUpperCase();
...
function renderScheduleTable() {
    ...
    workers.forEach((w, wIdx) => {
        ...
        const idStyle = grp ? `style="color:${themeColor}; font-weight:bold;"` : `style="opacity:0.7;"`;
        const nameStyle = grp
            ? `style="text-align:left; font-weight:bold; border-left: 3px solid ${themeColor}; padding-left: 8px; background: linear-gradient(90deg, ${themeColor}11, transparent);"`
...
function renderStaffList(filter = '') {
    ...
    list.innerHTML = sys_state.data.workers
        ...
        .map((w, idx) => {
            ...
            return `
                <div class="nav-link" style="border-left-width: 2px; ${borderStyle}" onclick="viewStaffDetails(${idx})">
                    <div style="width:100%">
